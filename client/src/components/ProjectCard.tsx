...
const ProjectCard = ({ gen, setGeneration, forCommunity = false }: {gen: Project, setGeneration: React.Dispatch<React.SetStateAction<Project[]>>, forCommunity?: boolean}) => {
    ...
    const handleDelete = async (id: string) => {
        const confirm = window.confirm("Are you sure you want to delete this generation? This action cannot be undone.");
        ...
        <div key={gen.id} className="mb-4 break-inside-avoid">
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
                ...
                >
                    ...
                    {/* Action Menu */}
                    {!forCommunity && (
                        <div onMouseDownCapture={() => {setOpenMenu(true)}} onMouseLeave={()=> {setOpenMenu(false)}} className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-2">
                            ...
                            <div className="flex flex-col items-end w-32 text-sm">
                                <ul
                                    ...
                                    } overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 py-1 z-10`}
                                >
                                    {gen.generatedImage && (
                                        <a
                                            href="#"
                                    ...
                                    {gen.generatedVideo && (
                                        <a
                                            ...
                                            className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer"
                                        >
                                            <PlaySquare size={14} /> Download Video
                                        </a>
                                    )}
                ...
                {/* Details */}
                    <div className="p-4">
                        ...
                        <div className="flex-1">
                            <h3 className="text-lg font-medium mb-1">{gen.productName}</h3>
